...
class LLMSettingsRepository:
        ...
        with connection, connection.cursor() as cursor:
            cursor.execute(
                ...
                SET provider = %s, endpoint_url = %s,
                    ...
                    resume_model = %s, scraping_model = %s,
    ...
    def _from_row(self, row: dict) -> LLMSettings:
        return LLMSettings(
            ...
            vision_model=row.get('vision_model', 'gpt-4.1-mini'),
            attribution_model=row['attribution_model'],
            review_model=row['review_model'],
            resume_model=row.get('resume_model', 'gpt-4.1-mini'),
            scraping_model=row.get('scraping_model', 'gpt-4.1-mini'),
