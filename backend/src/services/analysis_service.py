...
class AnalysisService:
    ...
    def detect_metadata(self, text: str) -> dict:
        ...
        if re.search(_ML_PATTERNS, text, re.IGNORECASE):
            category_slug = 'math'

        # ---- 强信号：纯数学 / 数论 / 几何 --> math ----
        if category_slug == 'simulation':
            _MATH_PATTERNS = (
                r'(质数|素数|质因数|最大公约数|最小公倍数|GCD|LCM|'
                r'排列组合|组合数|概率|期望|数论|'
                r'几何|三角形|矩形面积|最小外接圆|坐标系|行列式|'
                r'取模|模\s*\d|同余|费马|欧拉|'
                r'完全平方数|因子|水仙花数|蛇形数|斐波那契)'
            )
            if re.search(_MATH_PATTERNS, text):
                category_slug = 'math'

        # ---- 强信号：DP --> dynamic-programming ----
        if category_slug == 'simulation':
            dp_pat = (
        ...
        # ---- 强信号：回溯 --> backtracking ----
        if category_slug == 'simulation':
            if re.search(r'(回溯|全排列|N皇后|八皇后|解数独|括号生成)', text):
                category_slug = 'backtracking'

        # ---- 强信号：贪心 --> greedy ----
        if category_slug == 'simulation':
            if re.search(r'(贪心|最优策略|局部最优|最优选择)', text):
                category_slug = 'greedy'

        # ---- 强信号：位运算 --> bit-manipulation ----
        if category_slug == 'simulation':
            if re.search(r'(位运算|异或|XOR|按位与|按位或|bitwise|位操作|二进制操作)', text):
                category_slug = 'bit-manipulation'

        # ---- 强信号：二分 --> binary-search ----
        if category_slug == 'simulation':
            if re.search(r'(二分查找|二分答案|二分法|binary.search|折半查找)', text_lower):
                category_slug = 'binary-search'

        # ---- 强信号：双指针 --> two-pointers ----
        if category_slug == 'simulation':
            if re.search(r'(双指针|对撞指针|左右指针|快慢指针)', text):
                category_slug = 'two-pointers'

        # ---- 强信号：滑动窗口 --> sliding-window ----
        if category_slug == 'simulation':
            if re.search(r'(滑动窗口|长度为\s*k\s*的子|长度为\s*K\s*的子)', text):
                category_slug = 'sliding-window'

        # ---- 强信号：前缀和 --> prefix-sum ----
        if category_slug == 'simulation':
            if re.search(r'(前缀和|差分数组|prefix.sum)', text_lower):
                category_slug = 'prefix-sum'

        # ---- 强信号：堆 --> heap-priority-queue ----
        if category_slug == 'simulation':
            if re.search(r'(优先队列|大根堆|小根堆|大顶堆|小顶堆|最大堆|最小堆|topk|Top K)', text):
                category_slug = 'heap-priority-queue'

        # ---- 强信号：单调栈 --> monotonic-stack ----
        if category_slug == 'simulation':
            if re.search(r'(单调栈|下一个更大|下一个更小|上一个更大|上一个更小)', text):
                category_slug = 'monotonic-stack'

        # ---- 强信号：栈/队列 --> stack-queue ----
        if category_slug == 'simulation':
            if re.search(r'(括号匹配|表达式求值|后缀表达式|逆波兰|出栈|入栈|栈|堆栈|队列|双端队列|deque|单调队列)', text):
                category_slug = 'stack-queue'

        # ---- 强信号：链表 --> linked-list ----
        if category_slug == 'simulation':
            if re.search(r'(链表|单链表|双向链表|反转链表|合并链表|环形链表)', text):
                category_slug = 'linked-list'

        # ---- 强信号：区间 --> intervals ----
        if category_slug == 'simulation':
            if re.search(r'(合并区间|区间合并|重叠区间|区间调度|区间.*覆盖|合并.*区间)', text):
                category_slug = 'intervals'

        # ---- 强信号：矩阵/网格 --> matrix-grid ----
        if category_slug == 'simulation':
            if re.search(r'(螺旋矩阵|矩阵旋转|顺时针旋转|二维数组|棋盘)', text):
                category_slug = 'matrix-grid'

        # ---- 弱信号：关键词计分 ----
        if category_slug == 'simulation':
            _CATEGORY_KW: dict[str, list[str]] = {
                'two-pointers': ['双指针', '左右指针', '首尾指针', '两个指针'],
                'sliding-window': ['滑动窗口', '连续子数组'],
                'hashing': ['哈希', 'hash', '字典', '映射', '频次', '去重', '集合', '计数'],
                'binary-search': ['二分', '折半', '第k小', '第k大', '中位数'],
                'prefix-sum': ['前缀和', '区间和', '前缀'],
                'intervals': ['区间'],
                'matrix-grid': ['矩阵', '网格'],
                'linked-list': ['链表', '头结点', '尾结点'],
                'stack-queue': ['栈', '队列', 'deque'],
                'monotonic-stack': ['单调栈'],
                'heap-priority-queue': ['堆', 'topk'],
                'tree': ['树', '子树', '叶子', '根节点', '祖先', '深度', '直径'],
                'graphs': ['图', '邻接', '路径', '环路', '连通', '遍历'],
                'backtracking': ['回溯', '全排列', '组合', '子集', '排列', '枚举'],
                'dynamic-programming': ['dp', '动态规划', '递推', '状态转移', '背包'],
                'greedy': ['贪心', '最优策略', '局部最优'],
                'bit-manipulation': ['位运算', '异或', '按位', '二进制', '移位'],
                'simulation': ['模拟', '仿真', '构造', '按要求', '根据规则', '流程'],
                'math': ['数学', '公式', '方程', '函数', '多项式', '统计', '均值', '方差', '标准差'],
                'string': ['字符串', '字符', '回文', '模式匹配', 'KMP', '正则', '字母'],
            }
            max_score = 0
